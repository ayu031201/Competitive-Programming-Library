...
void simp(){
    ...
    for(int i=0;i<=n;i++){
        for(int j=0;j<=k;j++){
            ...
            // first is leaving that book
            // sum same rhega aur coin ek kam hojayega
            if(i==1){
                maxx1=0;
            }
            else{
            ...
            // taking the book 
            // sum-a[i] needed now and i-1 coins required
            if(a[i]>j){
                maxx2=0;
            }
            else{
                maxx2=b[i]+dp[i-1][j-a[i]];
            }
            dp[i][j]=max(maxx1,maxx2);
        }
    }
    cout<<dp[n][k]<<endl;
