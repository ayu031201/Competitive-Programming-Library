...
void dijkstra_2(int n){
    ...
    while(!pq.empty()){
        auto curr=pq.top();
        pq.pop();
        if(diss[curr.ss]!=curr.ff){
            continue;
        }
        for(auto x:vrev[curr.ss]){
            if(diss[x.ss]>x.ff+diss[curr.ss]){
                diss[x.ss]=x.ff+diss[curr.ss];
                pq.push(mp(diss[x.ss],x.ss));
            }
        }
    }
}
   ...
   // freopen("output.txt", "w", stdout);
        ...
        for(int i=1;i<=n;i++){
            for(auto x:v[i]){
                int curr=i;
                int coun=x.ss;
                minn=min(minn,dis[curr]+diss[coun]+x.ff/2);
            }
        }
        cout<<minn<<endl;
    }
